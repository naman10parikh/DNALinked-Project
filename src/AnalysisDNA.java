...
public class AnalysisDNA {
    ...
    /**
     * This method drives the program. It calls the method cutAndSplice.
     * It also calculates the time taken for the execution of the method cutAndSplice each time it is called.
     * The time returned is an average of execution time of cutAndSplice (the method is called TRIALS times)
     * @param enzyme
     * @param splicee
     * @param className
     * @return ret
     * @return "could not create class " + className;   if an exception is thrown
     * @throws Exception
     */
    public static String strandSpliceBenchmark(String enzyme, String splicee, String className)
        ...
        try {
            ...
            String ret = String.format("%s:\t%,15d\t%,15d\t%1.3f\t%d", className, splicee.length(), recLength, time, recomb.getAppendCount());
     ...
     * Reads the DNA data from the file and calls the method StrandSpliceBenchmark() for different values of mySource
     * Prints output
     * @param args
     * @throws Exception
     */
    public static void main(String[] args)
        ...
        // Run code 10 times, change number of occurrences on enzyme (linear)
        for (int j = 0; j<10; j++){
            mySource += copy;
